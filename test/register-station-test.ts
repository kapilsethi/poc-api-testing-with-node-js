...
describe("Register station -->", async () => {
  ...
  const apiKey =
    testEnv === "prod" ? process.env.API_KEY : testData[testEnv]["validApiKey"];
  ...
  describe("Success scenario --> ", async () => {
    ...
    it("should be able to delete the registered station", async () => {
      ...
      const response = await apiHelper.deleteRequestMethod(updatedUrl, apiKey);
      expect(
        response.status,
    ...
    it("should not be getting station as its been deleted", async () => {
      ...
      const response = await apiHelper.getRequestMethod(
        updatedUrl,
        updatedApiKey
      );
      expect(
